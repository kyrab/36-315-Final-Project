...
The clusters of this dendrogram, made based off of the three variables described above, are not just the five main genres. In other words, it appears to show very little difference between the five genres. Each of the clusters' leaves, colored by Genre, appear to somewhat similarly distributed, with the majority of the distribution consisting of rock songs.
...
What qualities do popular songs emobody? By taking a look at the correlation between Popularity 

# no correlation between popularity and other variables

### Graph 1


```{r}
#ggpairs(data = spotify, columns = (8:15))

ggplot(spotify, aes(x = Genre, y = Popularity)) + 
  geom_boxplot() + 
  labs(title = "Popularity across Genres")

ggplot(spotify, aes(x = Decade, y = Popularity, fill = Genre)) + 
  geom_boxplot() + 
  labs(title = "Popularity across Decades")
```
...
Overall, there is not much of a difference between the five genres, at least in terms of the quantitative variables in this dataset. While we did observe some minor trends in Energy, Danceability, and Popularity, where one or two genres somewhat differentiated themselves from the rest, we could not observe any clear difference between them.

We were able to conclude that Popularity is not assoicated with any of the other qualitative variables in the dataset. While indie music is less popular than the other genres, there are no differences between the rest.

Finally, we can see that a variety of qualitative and quantitative attributes have appeared to change over time.

In our future work, we look forward to exploring these relationships with greater granularity and experimenting with various subsets of the data to perform subgroup analyses.
